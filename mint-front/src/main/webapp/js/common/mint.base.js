 ...
 *                         }
 * @param hasRouter     : Restful Service 라우터 사용 구분 (기본값 옵션 : false), true 로 바꾸면 라우터를 사용하게 된다.(2020.10 추가)
 */
mint.callService = function(
	...
	try {
		...
		//----------------------------------------------------------------------
		{
			var envVal = mint.common.getEnvValue('system.hasRouter');
			var hasRouter = (!mint.common.isEmpty(envVal) && envVal === 'Y') ? true : false;
		}

		//----------------------------------------------------------------------
		// Service URL
		//----------------------------------------------------------------------
		var url = mint.rest.getServiceUrl(restId, hasRouter);
		if( !url ) {
